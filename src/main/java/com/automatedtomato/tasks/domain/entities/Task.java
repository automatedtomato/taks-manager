package com.automatedtomato.tasks.domain.entities;
...
public class Task {
    ...
    @Column(name = "id, updated = false, nullable = false")
    private UUID id;  // UUIDは128bitの固定長のバイナリデータ
    ...
    @ManyToOne(fetch = FetchType.LAZY)  // fetch = FetchType.LAZY -> TaskListは必要なときだけDBから取得
    @JoinColumn(name = "task_list_id")
